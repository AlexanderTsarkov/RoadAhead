...
8. **Route providers may supply geometry, not RoadAhead behavioral truth.** When explicitly used, route-provider geometry may be treated as a route-geometry input. Provider speed, ETA, traffic speed, segment speed, posted-limit data, lane / turn instructions, and other non-geometry signals are not RoadAhead product truth and must not be presented as such.
9. **RoadAhead guidance must be conservative when applicability or data truth is ambiguous.** When RoadAhead cannot confidently determine that a candidate event applies to the current path and direction, or when candidate data truth is in doubt, the driver-facing posture is suppression / non-claim over confident display. More specific ambiguity classes and debug handling belong to the relevant event-applicability / data / UI areas.
...
- **Do not present route-provider non-geometry data as RoadAhead truth.** When a route provider supplies geometry, only the geometry feeds RoadAhead logic; provider speed, ETA, traffic speed, segment speed, posted-limit data, lane / turn instructions, and other non-geometry signals must not flow into RoadAhead's driver-facing values. (From truth 8.)
- **When data applicability is ambiguous, prefer suppression / non-claim over confident driver-facing display.** Candidates that cannot be confidently attributed to the current path / direction, or whose data truth is in doubt (for example wrong-road, wrong-direction, or branch-ambiguous cases), are suppressed in the driver-facing event selection by default. Suppressed / ambiguous candidates should remain inspectable in debug / QA surfaces where available, so suppression is auditable and tunable; concrete debug shape and ambiguity classification belong to the relevant event-applicability / data / UI areas, not to this boundary-level rule. (From truth 9.)
...
- **Truth 8 — Route providers may supply geometry, not RoadAhead behavioral truth.**
