...
- **Yandex** should remain the **primary candidate** for Russia-focused route geometry **if and only if** its current API/key/terms/pricing constraints turn out to be acceptable for a local POC. Yandex's HTTP Router API and JS API both expose a clean route polyline that fits the §2 contract below. The conditional posture is deliberate; Yandex commits the POC to an online dependency, an API key, a domain restriction model, and explicit caching limits (see §3.A and §7).
...
- **ordered polyline coordinates** along the route, in a consistent `(lat, lon)` or `(lon, lat)` convention chosen once and applied everywhere downstream;
...
- The HTTP Router API and JS API both produce a polyline that satisfies the §2 contract (ordered `(lon, lat)` vertices in WGS84). Either could feed the normalized RouteGeometry through an adapter.
- The JS API removes the need for a server-side proxy and is the most direct path to "browser-only POC". The HTTP Router API would normally be considered server-side; using it directly from the browser would expose the API key in client code, which the API key restrictions partially mitigate (Referer / IP filtering) but do not fully solve. A small local proxy is the conservative pattern, but a proxy is itself infrastructure and is **not** approved by this document. The proxy-vs-direct decision belongs to a later implementation slice.
...
- **GeoJSON** (RFC 7946) represents a route most directly as a `Feature` with `geometry.type = "LineString"` and `coordinates` as an ordered array of `[lon, lat]` pairs (optionally `[lon, lat, alt]`). It is plain JSON, trivially parseable in browsers without extra dependencies. (GeoJSON is referenced here as a well-known interchange format; no provider-specific URL is required.)
...
- **Coordinate convention** (`(lat, lon)` vs `(lon, lat)`) is chosen once at the adapter exit and applied consistently. Mixing conventions is a known source of subtle bugs.
...
- **Document whether route geometry can be cached locally** in the emulator. For Yandex, short-lived in-memory / in-session caching is acceptable under the 30-day temporary-caching clause; persistent on-disk caching of Yandex routes is **not** approved by this document. For GPX/KML/GeoJSON files the user loads themselves, local persistence is fine because the user owns the file.
...
- Can a route be **saved and reloaded** as a GeoJSON or GPX file, so the emulator can replay the same scenario without re-hitting the provider every time? This is a key reliability test for Yandex specifically given the 30-day caching limit.
